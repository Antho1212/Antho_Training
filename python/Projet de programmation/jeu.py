...
def order(player_ID,message:str) -> dict:
    ...
    try:
        ...
        #if there is an error we return an empty value
        return
...
def IA_attack(player):
    """select dragons that IA can play and use a random attack
    Parameters:
    -----------
    player: player which AI is (int)
    Return:
    -----------
    command: attack's order (int)
    Version:
    --------
    Specification : Sacha Ismailov (v.1 - 22/3/2025)
    Specification : Sacha Ismailov (v.2 - 22/3/2025)
    ...
    for dragon in dragons:
        if dragons[dragon]['joueur'] == player:
            playable_dragon += [dragon]
    try:
        choosed_dragons = random.choice(playable_dragon)
        choosed_attack = random.choice(["N","NE", "E","SE", "S", "SW","W","NW"])
        command = f' {choosed_dragons}:x{choosed_attack}'
        return command
    except:
        return ''

def IA_mouvement(type,player):
    """select dragons that IA can play and move randomly
    Parameters:
    -----------
    type: type of entity (dragons or apprentice) (dict)
    player: player which AI is (int)
    Return:
    -----------
    msg: mouvement's order
    Version:
    --------
    Specification : Sacha Ismailov (v.1 - 22/3/2025)
    Specification : Sacha Ismailov (v.2 - 22/3/2025)
    ...
    playable_entyty = []
    for entyty in type:
        if type[entyty]['joueur'] == player:
            playable_entyty += [entyty]
    entity = random.choice(list(type))
    x = type[entity]['position_x']
    y = type[entity]['position_y']
    mvmt_x = x + random.choice(list([1,-1]))
    mvmt_y = y + random.choice(list([1,-1]))
    msg = f' {entity}:@{mvmt_y}-{mvmt_x}'
    return msg
...
def get_AI_orders(game, player_id):
    ...
    orders = IA_attack(player_id)
    orders += IA_mouvement(dragons,player_id)
    orders += IA_mouvement(apprentices,player_id)
...
"""
map_data, altars, apprentices, dragons, special_capacity = lecture_file('/Users/sacha_ismailov/Codes/Projet_Drakon-main copie/game_data.drk')
# main function
type_1 = 'in front of the pc'
type_2 = ''
...
def in_attack_range(apprentices: dict, dragons: dict, dragon: str, directions = None) -> list:   
    ...
    for direction in directions:  # Check for every direction
        for x in range(1, range_attack + 1):  # Check the tile in range
            ...
            if check_people_on_case(x_attack, y_attack):  # Check if there is someone on the case
...
def sniper_to_cible(apprentices: dict, dragons: dict):
    ...
    min_distance = None
    best_distance = None
    if sniper in apprentices and cible in dragons:
        for sniper in apprentices:
            for cible in dragons:
            # vérifier si le dragon n'appartient à aucun n'apprenti ainsi ça montre que s'est un oeuf
                if cible not in apprentices[sniper]['joueur']: #reviznt à dire "if dragons[cible]["joueurs"] is None:""
                    # si le sniper c'est l'apprentit il va vers l'oeuf
                            if apprentices[sniper]['joueur'] == dragons[cible]['joueur']: #vérifie qu'il se dirige vers l'ennemiµ
                                #se dirige vers l'oeuf le plus rentable pour gagner la game
                                best_oeuf = best_rentability_move(sniper, cible)
        #MOUVEMENT
        ...
        if best_oeuf in dragons : #vérifie si le dragons est bien dans le dictionnaire des dragons
            ...
            if sniper_y < cible_y:
                ...
                sniper_y -= 1
            msg = f' {sniper}:@{sniper_y}-{sniper_x}'
            return msg
                
    elif sniper in dragons and cible in apprentices:
        for sniper in dragons:
            for cible in apprentices:
            #  si le sniper c'est le dragon il va vers l'apprenti 
                #trie les dragons avec les meilleur stat pour ensuite se diriger vers les dragons ennemies avec les pires stat
                tri_dragons =choose_good_dragons(dragons)
                middle_choose_good_dragon= len(dragons) // 2
                worst_dragon_until_the_middle = tri_dragons[middle_choose_good_dragon:]   #lire la list à moitié pour se dirigé uniquement vers les drgons faible
                bad_dragons = []
                for dragon in worst_dragon_until_the_middle:
                    name=dragon[1] #pas sûr que ça soit position 1 ou 0 pas clair ton retour Sacha
                    bad_dragons+= name

                if cible in bad_dragons:
        ...
        if best_distance :
            sniper_x = apprentices[sniper]['position_x']
            sniper_y = apprentices[sniper]['position_y']
            cible_x = dragons[best_distance]['position_x']
            cible_y = dragons[best_distance]['position_y']
            choosed_attack = get_direction(apprentices, dragons,sniper, cible)
            command = f' {sniper}:x{choosed_attack}'
            return command
    return None



def best_rentability_move(apprentices: dict, dragons: dict):
...
def attack(apprentices: dict, dragons: pygame.event.__dict__):
    """attack of the IA
    Parameters
    ----------
    dragons (dict): data about dragons
    ...
    Return:
    -------
    order (str): the attack order
    Version:
    --------
    Specification: Anthony Goffin (v.1 - 10/04/2025)
    Implementation: Anthony Goffin (v.1 - 10/04/2025)
    """
    for dragon in dragons:
        if in_attack_range(apprentices,dragons) != []:
            direction = get_direction(apprentices, dragons)
            msg = dragon,":",direction
            message += msg + " "
            return message
    return None
