...
def in_attack_range(apprentices: dict, dragons: dict, dragon: str, directions = None, friends = False) -> list:   
    """
    ...
    Friends (bool): if you want youre team in the list range or not, basically it's false
    Returns
    -------
            ...
            if check_people_on_case(x_attack, y_attack):  # Check if there is someone on the case
                ...
                for other_dragon in dragons:  # Same for the dragons
                    ...
                    if (x_other_dragon == x_attack) and (y_other_dragon == y_attack) and (other_dragon not in people_on_range):
                        if dragons[other_dragon]["joueur"] != dragons[dragon]["joueur"]:
...
def apprentices_to_cible(apprentices: dict, dragons: dict):
...
def dragons_to_cible(apprentices: dict, dragons: dict):
    """finds the target closest to the sniper
...
def attack_orders(apprentices: dict, dragons: dict)->str:
    ...
    for dragon in dragons:  # Loop through each dragon
        ...
        if in_range:  # If there are enemies in range
            ...
            in_range_direction = in_attack_range(apprentices, dragons, dragon, direction)
            if attack_is_good():
                order_message += f"{dragon}:{direction} "  # Add to the attack order
    
    return order_message


def attack_is_good(apprentices:dict, dragons: dict, in_range_direction: str):   #analyse si un coup est bon calcul de la value, nombre d'ennemie, nombre de pote                                      
    ...
    dragons (dict): data about dragons
    in_range_directional (list): list of ennemie in the attack range in the target direction
    returns
    -------
...
def get_AI_orders(game, player_id):
    ...
    orders = dragons_to_cible(apprentices, dragons)
    orders += apprentices_to_cible(apprentices, dragons)
    orders += attack_orders(apprentices, dragons)
