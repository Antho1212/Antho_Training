...
def apprentices_to_cible(apprentices: dict, dragons: dict, message: str, player_id: int):
            ...
            if ':' in command:
                ...
                if (apprentices[name]['joueur'] == player_id):
                    if action and action[0] == '@' and '-' in action:
                        coordonnee = action[1:].split('-')  #Divide the coordinates using the (-) and do action[1:] because there are coordinates with a single letter and others with several like "NW"
                        position_occupe  += (int(coordonnee[0]), int(coordonnee[1]))
        ...
        if apprentices[sniper]['joueur'] == player_id:
            ...
            if best_oeuf in dragons : #vérifie si le dragons est bien dans le dictionnaire des dragons
                ...
                if (new_sniper_x, new_sniper_y) not in position_occupe:
                    ...
                    # ce message est récupérer de oeuf_occupe et permet de se déplacer vers les oeufs pas encore occupé
                    message += msg + " "
    return message
...
def best_rentability_move(apprentices: dict, dragons: dict, sniper: str):
    ...
    for dragon in dragons:  # Loop through each dragon
        if dragons[dragon]['joueur'] == player_id:
            ...
            if in_range:  # If there are enemies in range
                ...
                in_range_directional = in_attack_range(apprentices, dragons, dragon, direction, True) #get the entity in this range
...
def oeufs_occupes(apprentices: dict, dragons: dict) -> dict:
    """
    1 L'apprenti va vers l'œuf
    2 un apprenti ne quitte pas une case contenant un œuf tant que celui-ci n'a pas éclos.
    3 un œuf ciblé par un apprenti ne peut être ciblé par un autre.
    4 Si un apprenti est sur une case, ni un autre apprenti ni un dragon ne peut y aller.
    parameters
    ----------
    apprentices: apprentice data dictionary (dict)
    dragons: dragons data dictionary (dict)
    returns
    -------
    occupe: dictionnaire des apprenti assigné a un oeuf
    """
    occupe = {}
    positions_des_oeufs = []
    cases_deja_prises = []
    ...
    message = ""
    # on récupère les oeufs sans les dragons
    # première étape on récupère les oeufs
    for dragon in dragons:
        if dragons[dragon]['joueur'] is None:
             # on ajoute à la liste tout les oeuf disponible
            positions_des_oeufs += [(dragons[dragon]['position_x'], dragons[dragon]['position_y'])]
    print("occupé à la base :", occupe)
    # on traite chaque œuf
    # deuxième étape Un apprenti ne quitte pas une case contenant un œuf tant que celui-ci n'a pas éclos
    for cible in dragons:
        if dragons[cible]['joueur'] is None:
            apprenti_sur_la_case = []
            x = dragons[cible]['position_x']
            y = dragons[cible]['position_y']
            # vérification si l'apprenti est déjà sur la case
            for sniper in apprentices:
                if apprentices[sniper]['position_x'] == x and apprentices[sniper]['position_y'] == y:
                    apprenti_sur_la_case += [sniper]

            if len(apprenti_sur_la_case) == 1:
                #si un seul apprenti sur la case, il garde l'œuf
                occupe[(x, y)] = apprenti_sur_la_case[0]
                cases_deja_prises += [(x, y)] #ici on bloque la condition empêchant au autre apprenti d'y aller
                """print(f"l'apprenti {apprenti_sur_la_case[0]} a pris l'œuf à la position {(x, y)}")
                print("occupé maintenant :", occupe)"""

            elif len(apprenti_sur_la_case) > 1:
                 # on va vers l'oeuf le plus proche si un apprenti est déjà dessus, en chossissant le meilleur oeuf
                best_oeuf = None
                best_stat = -1
                ...
                cases_deja_prises += [(x, y)]
                """print(f"l'apprenti {apprenti_sur_la_case[0]} a pris l'œuf à la position {(x, y)}")
                print("occupé maintenant :", occupe)"""
                # les apprenti pas choisi par la condition sont déplacer
                for apprenti in apprenti_sur_la_case:
    ...
    # troisième étape Un œuf ciblé par un apprenti ne peut être ciblé par un autre.
    # quatrième étape Si un apprenti est sur une case, ni un autre apprenti ni un dragon ne peut y aller.
    # on gère les apprentis restants
    for apprenti in apprentices:
        x = apprentices[apprenti]['position_x']
        y = apprentices[apprenti]['position_y']
        trouve = False
        # on vérifie si l'apprenti est déjà seul sur un œuf (alors il ne bouge pas)
        for case in occupe:
            if occupe[case] == apprenti and case == (x, y): #si les noms correspondent et les coordonnées sont les meme bloqué l'apprenti sur la case
                trouve = True
        if not trouve: #pareil que trouve :: False
            # on cherche un œuf libre
            distance_min = 9999
            oeuf_cible = None
            for oeuf in positions_des_oeufs:
                if oeuf not in cases_deja_prises: #donc si l'oeuf existe et qu'il n'est pas encore pris y aller
                    distance = abs(apprentices[apprenti]['position_x'] - oeuf[0]) + abs(apprentices[apprenti]['position_y'] - oeuf[1])
                    if distance < distance_min:
                        distance_min = distance
                        oeuf_cible = oeuf
            if oeuf_cible is not None:
                occupe[oeuf_cible] = apprenti
                cases_deja_prises += [oeuf_cible]
                # déplacement vers l'œuf cible
                sniper_x =apprentices[apprenti]['position_x']
                sniper_y =apprentices[apprenti]['position_y']
                cible_x, cible_y = oeuf_cible
                if sniper_x < cible_x:
...
def checkbestdefence(player):
    ...
    for badguy in dragons:
        people_on_range = in_attack_range(apprentices, dragons, player) # Donne la liste des personnes menacées
        if len(people_on_range) > len(apprentices[ally]):
            special_capacity_phase(player) # Fait son action special
        elif len(people_on_range) == len(apprentices[ally]):
            ...
            defencemove(ally, enemy)

def defencemove(ally, enemy):
    ...
    for badguy in dragons:
        for novice in apprentices:
            people_on_range = in_attack_range(apprentices[ally], apprentices[ally][dragons], apprentices[enemy][dragons][badguy])
            if people_on_range == apprentices[ally][novice]: # Regarde si il y a des apprentis allié dans la liste des personnes menacée
                if apprentices[ally][novice]['health'] < apprentices[enemy][dragons][badguy]['attack']: # Vérifie si l'attaque de l'ennemi est plus grand que la vie de l'apprenti
                    for direction in listfor:
                        in_attack_range(apprentices[ally], apprentices[ally][dragons], apprentices[enemy][dragons][badguy], direction) # Pour avoir people_on_range
                        if people_on_range == apprentices[ally][novice]: #Voir si il y a qqn dans la direction
                            row = apprentices[ally][novice]["position_x"]
                            column = apprentices[ally][novice]["position_y"]
                            if direction == "N" or direction == "S":
                                if row > 15: #Regarde si les entitées sont plus à droite ou à gauche
                                    row += -1
                                    movement_phase(apprentices[ally],None,"@"+ row + column ,None) # Fuir de l'attaque en tant que défence (Refaire)
                                else:
                                    row += 1
                                    movement_phase(apprentices[ally],None,"@"+ row + column ,None) # Fuir de l'attaque en tant que défence (Refaire)
                            elif direction == "W" or direction == "E":
                                if column > 15: #Regarde si les entitées sont plus en haut ou en bas
                                    column += 1
                                    movement_phase(apprentices[ally],None,"@"+ row + column ,None) # Fuir de l'attaque en tant que défence (Refaire)
                                else:
                                    column += -1
                                    movement_phase(apprentices[ally],None,"@"+ row + column ,None) # Fuir de l'attaque en tant que défence (Refaire)
                            elif direction == "NW" or direction == "NE":
                                column += -1
                                row += 1
                                movement_phase(apprentices[ally],None,"@"+ row + column ,None)
                            elif direction == "SW" or direction == "SE":
                                column += 1
                                row += -1
                                movement_phase(apprentices[ally],None,"@"+ row + column ,None)
                            else:
                    ...
                    apprentices_to_cible(apprentices[ally],apprentices[enemy][dragons][badguy]) # Effectue la fonction d'attaque
            elif people_on_range == apprentices[ally][dragons]: # Regarde si il y a des dragons allié dans la liste des personnes menacée
                if apprentices[ally][dragons]['health'] < apprentices[enemy][dragons][badguy]['attack']: # Vérifie si l'attaque de l'ennemi est plus grand que la vie du dragon
                    for direction in listfor:
                        in_attack_range(apprentices[ally], apprentices[ally][dragons], apprentices[enemy][dragons][badguy], "N")
                        if people_on_range == apprentices[ally][dragons]: #Voir si il y a qqn au nord (A REFAIRE)
                            row = apprentices[ally][novice][dragons]["position_x"]
                            column = apprentices[ally][novice][dragons]["position_y"]
                            if direction == "N" or direction == "S":
                                if row > 15: #Regarde si les entitées sont plus à droite ou à gauche
                                    row += -1
                                    movement_phase(None,apprentices[ally][novice][dragons],"@"+ row + column ,None) # Fuir de l'attaque en tant que défence (Refaire)
                                else:
                                    row += 1
                                    movement_phase(None,apprentices[ally][novice][dragons],"@"+ row + column ,None) # Fuir de l'attaque en tant que défence (Refaire)
                            elif direction == "W" or direction == "E":
                    ...
                    dragons_to_cible(apprentices[ally][dragons],apprentices[enemy]) # Effectue la fonction d'attaque
            else:
                return None
